    ...
    __out DWORD *pdwCodePtr, __out DWORD *pdwSizeOfData, __out DWORD *pdwCodeSecVirtAddr)
{
    ...
    if ((pImgSecHeader = GetEnclosingSectionHeader(pNTHeaders->OptionalHeader.AddressOfEntryPoint,
        pNTHeaders)) == NULL)
    {
        wprintf(L"GetPtrToCode(): pImgSecHeader NULL\n");
        return FALSE;
    }
    ...
    {
        wprintf(L"Retrieved section header %S but does not contain code or is not executable\n",
...
BOOL Util_DumpIMAGE_IMPORT_DESCRIPTORS(DWORD rva, DWORD dwSize, PIMAGE_NT_HEADERS pNTHeaders, DWORD dwFileBase)
{
    ...
    {
        wprintf(L"fDumpIMAGE_IMPORT_DESCRIPTOR(): Unable to retrieve section header\n");
        wprintf(L"rva = 0x%xh, dwSize = 0x%xh, pNTHeaders = %p, dwFileBase = 0x%08xh\n",
    ...
    {
        wprintf(L"Entry:\n");
        wprintf(L"    Char/OrigFirstThunk: %x\n", pImports->OriginalFirstThunk);
        if (pImports->TimeDateStamp != 0)
        {
            wsprintfW(wszTimeStamp, _wctime((time_t*)&pImports->TimeDateStamp));
            wprintf(L"    TimeDateStamp:       %xh %s\n", pImports->TimeDateStamp, wszTimeStamp);
        }
        wprintf(L"    ForwarderChain:      %xh\n", pImports->ForwarderChain);
        if (pImports->Name)
        {
            wprintf(L"    Name:                %S\n", (char*)pImports->Name + iFilePtrRVADelta + dwFileBase);
        }
        wprintf(L"    FirstThunk:          %xh\n", pImports->FirstThunk);
        wprintf(L"    IAT:\n");
